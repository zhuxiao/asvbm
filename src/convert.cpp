...
void convertAlleleBed(const string &infilename, const string &outfilename, string &mate_filename, string &snv_filename, string &label){
	ifstream infile;
	string line, lineInfo;
	vector<string> header1_lower, header2_lower;
	int header_count = 0;
	...
	struct NormalIndex {
		...
		size_t end = (size_t)-1;
		size_t svtype = (size_t)-1;
		size_t svlen = (size_t)-1;
		size_t ref = (size_t)-1;
		size_t alt = (size_t)-1;
	} normal_idx;

	struct TraIndex {
		size_t chr1 = (size_t)-1;
		size_t start1 = (size_t)-1;
		size_t end1 = (size_t)-1;
		size_t chr2 = (size_t)-1;
		size_t start2 = (size_t)-1;
		size_t end2 = (size_t)-1;
		size_t svtype = (size_t)-1;
		size_t svlen1 = (size_t)-1;
		size_t svlen2 = (size_t)-1;
		size_t ref1 = (size_t)-1;
		size_t alt1 = (size_t)-1;
		size_t ref2 = (size_t)-1;
		size_t alt2 = (size_t)-1;
	} tra_idx;
	...
	int32_t sv_len = 0, sv_len1 = 0, sv_len2 = 0;
	vector<string> str_vec;
	vector<SV_item*> sv_item_vec;
	SV_item *sv_item, *sv_item1;
	...
	while (getline(infile, line)) {
		...
		if (line.at(0) == '#' && header_count < 2) {
			...
			for (const auto &col : cols) {
				string col_clean = col;
					...
					if (header_count == 1) {
							header1_lower.push_back(col_lower);
						} else if (header_count == 2) {
							header2_lower.push_back(col_lower);
						}
			}
			if (header_count == 1) {
				...
				if (normal_idx.chr == (size_t)-1 || normal_idx.start == (size_t)-1 || normal_idx.end == (size_t)-1) {
					cerr << __func__ << ": error（Chr/Start/End）" << endl;
					exit(1);
				}
				...
				if (tra_idx.chr1 == (size_t)-1 || tra_idx.start1 == (size_t)-1 || tra_idx.end1 == (size_t)-1 ||
					...
					cerr << __func__ << ": error（Chr1/Start1/End1/Chr2/Start2/End2）" << endl;
					exit(1);
				}
			}
			if (label == "benchmark") {
		...
		bool is_normal = (header_count >= 1 && num_cols == header1_lower.size());
		bool is_tra = (header_count >= 2 && num_cols == header2_lower.size());
		if (is_normal) {
			if (normal_idx.chr >= num_cols || normal_idx.start >= num_cols || normal_idx.end >= num_cols) {
				cerr << __func__ << ": error: " << line << endl;
				continue;
			}
...
//            if (chromosomeSet.empty() && isDecoyChr(chrname)) continue;
			...
			empty_str.clear();
			string BND_type_str = "BND", BND_alt_seq = "-";
			if(sv_len > BND_MAX_SVLEN && sv_type_str.compare("INS")!=0){
				sv_item_vec.push_back(allocateSVItem(
						chrname, start_pos, start_pos, chrname, endpos, endpos,
						BND_type_str, 0, ref_seq, BND_alt_seq, lineInfo, label
					));
				sv_item_vec.push_back(allocateSVItem(
						chrname, endpos, endpos, chrname, start_pos, start_pos,
						BND_type_str, 0, ref_seq, BND_alt_seq, lineInfo, label
					));
			}else if(sv_len > BND_MAX_SVLEN && sv_type_str.compare("INS")==0){
				sv_item_vec.push_back(allocateSVItem(
						chrname, start_pos, start_pos, chrname, start_pos + sv_len, start_pos + sv_len,
						BND_type_str, 0, ref_seq, BND_alt_seq, lineInfo, label
					));
			}else{
...
//            sv_item_vec.push_back(sv_item2);
			...
			cerr << __func__ << ":（" << num_cols << "）error：" << header1_lower.size()
				 << "：" << header2_lower.size() << ": " << line << endl;
			continue;
		}
	}
...
//    if (!mate_filename.empty()) removemateSVItems(mate_filename, sv_item_vec);
	...
	Alleleoutput2File(outfilename, sv_item_vec, outConvertScreenFile);
	releaseSVItemVec(sv_item_vec);
...
void convertBed(const string &infilename, const string &outfilename, const string &reffilename, string &mate_filename, string &snv_filename, string &label) {
	...
	fai = fai_load(reffilename.c_str());
	if (!fai) {
		cerr << __func__ << ", line=" << __LINE__ << ": cannot open file:" << reffilename << endl;
		exit(1);
	}
...
//            if (chromosomeSet.empty() && isDecoyChr(chrname)) continue;
			...
			if(sv_len > BND_MAX_SVLEN && sv_type_str.compare("INS")!=0){
				if(sv_type_str.compare("DEL")==0){
					...
					reg_str = chrname + ":" + to_string(endpos) + "-" + to_string(endpos);
					seq = fai_fetch(fai, reg_str.c_str(), &refseq_len_tmp);
...
//				if(sv_type_str.size()==0 and sv_len_str.size()==0 and endpos_str.size()==0)	continue;
				...
				if(endpos_str.size()==0){
					if(sv_len_vec.size()==1){
						...
						if(sv_len_str.size()>0){
							if(sv_type_str.compare("INS")==0)
								endpos_str = to_string(stoi(start_pos_str) + ref_seq_len - 1);
							else
								endpos_str = to_string(stoi(start_pos_str) + stoi(sv_len_str) - 1);
						}else{
							if(sv_type_str.compare("INS")==0)
								endpos_str = to_string(stoi(start_pos_str) + ref_seq_len - 1);
							else
								endpos_str = to_string(stoi(start_pos_str) + abs(sv_len_vec.at(0)) - 1);
						}
...
void convertAlleleVcf(const string &infilename, const string &outfilename, string &mate_filename, string &snv_filename, string &label){
	ifstream infile;
	string line, chrname, start_pos_str, endpos_str, endpos_str_1, endpos_str_2, chrname2, start_pos_str2, endpos_str2, sv_type_str, sv_type_str1, sv_type_str2, sv_len_str, sv_len_str1, sv_len_str2, bnd_str, bnd_pos_str, str_tmp;
	string ref_seq, alt_seq, reg_str, lineInfo;
	size_t start_pos, endpos, endpos_1, endpos_2, start_pos2, endpos2;
